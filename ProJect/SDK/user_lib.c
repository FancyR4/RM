...
//区间映射函数
float float_map(float Inmin, float Inmax, float Outmin, float Outmax, float IN)
{
		if(Inmin >= 0)
		{
			float mapproportion;
			float OUT; 
			mapproportion = (Outmax - Outmin)/(Inmax - Inmin);
			OUT = mapproportion*IN;
			OUT = OUT>Outmax ? Outmax:(OUT<Outmin ? Outmin:OUT);
			return OUT;
		}
//		if(Inmin < 0)
//		{
...
//			float OUT;
//			//float OFFSET;
//			mapproportion = (Outmax + abs(Outmin))/(Inmax - Inmin);
//			OUT = mapproportion*IN;
//			//OFFSET = Inmin
//			OUT = OUT- 2*Outmin;
//			OUT = OUT>Outmax ? Outmax:(OUT<Outmin ? Outmin:OUT);
...
//区间选择函数
float float_Interval_Select(float IN, int N)
{
	float Interval[2];	
	//float **IntervalSet = (float*)malloc(N*sizeof(Interval));
