...
//fp32 gyro_cali_offset[3] ={0.00992421896f, -0.0259684507f, -0.0159789659f};0.0133158062
fp32 gyro_cali_offset[3] ={-0.0252295085f, 0.0138484379f, -0.0133158062f};
//fp32 gyro_cali_offset[3] = {0.0f, 0.0f, 0.0f};
fp32 Gyro_Offset[3] = {0.0f, 0.0f, 0.0f};            //陀螺仪零漂 0.00992421896f, -0.0239684507f, -0.0159789659f
...
void PID__Init(PidTypeDef *pid, uint8_t mode, const fp32 PID[3], fp32 max_out, fp32 max_iout)
{
...
//将读取到的ist8310原始数据处理成国际单位的数据
        ...
        IMU_Cali_Slove(INS_gyro, INS_accel, INS_mag, &mpu6500_real_data, &ist8310_real_data);
				...
				for(int i = 0;i < 3;i++)
				{
					if((INS_gyro[i] > -0.005) && (INS_gyro[i] < 0.005))
					{
							INS_gyro[i] = 0;
					}
				}
        ...
        if( mpu6500_real_data.status & 1 << MPU_DATA_READY_BIT)
        {
            ...
            if (updata_count == 0)
            {
                ...
                PID__Init(&imuTempPid, PID_DELTA, imuTempPID, MPU6500_TEMPERATURE_PID_MAX_OUT, MPU6500_TEMPERATURE_PID_MAX_IOUT);
