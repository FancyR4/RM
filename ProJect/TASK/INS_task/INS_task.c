...
//将读取到的ist8310原始数据处理成国际单位的数据
        ...
        if( mpu6500_real_data.status & 1 << MPU_DATA_READY_BIT)
        {
            ...
            if (updata_count == 0)
            {
                ...
                IMUPID_Init(&imuTempPid, PID_DELTA, imuTempPID, MPU6500_TEMPERATURE_PID_MAX_OUT, MPU6500_TEMPERATURE_PID_MAX_IOUT);
...
static void IMU_temp_Control(fp32 temp)
{
    ...
    if (first_temperate)
    {
        IMUPID_Calc(&imuTempPid, temp, INS_GET_CONTROL_TEMPERATURE());
